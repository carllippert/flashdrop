...
import {ISuperfluid, ISuperToken, ISuperApp, ISuperAgreement, SuperAppDefinitions, IERC20} from "@superfluid-finance/ethereum-contracts/contracts/interfaces/superfluid/ISuperfluid.sol";
...
contract RedirectAll is SuperAppBase {
   ...
   //each nft minted creates a pool, the uuid generated on the client because we need to 
  //embed data to call the pool for claim page in qrcode which transalates to
...
mapping(string => Flashdrop) public flashDrops;
    ...
    function _endFlashDrop(string memory uuid) internal {
        //we need to call this so we do not go to SuperApp Jail
    }
